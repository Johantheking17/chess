...
public class King extends Piece {
	...
	public ArrayList<Square> getLegalMoves(Board b, Square currentSquare) {
		...
		//left
		if (col > 0) {
			if (!board[row][col-1].isOccupied() || board[row][col -1].getOccupyingPiece().getColor() != this.getColor() && !(board[row][col-1].getOccupyingPiece() instanceof Unmovable)) {
				legalMoves.add(board[row][col-1]);
			}
			//left up
			if (row > 0) {
				if (!board[row -1 ][col -1].isOccupied()
						|| board[row - 1][col - 1].getOccupyingPiece().getColor() != this.getColor() && !(board[row - 1][col - 1].getOccupyingPiece() instanceof Unmovable)) {
		...
		//up
		if (row > 0) {
			if (!board[row-1][col].isOccupied() || board[row-1][col].getOccupyingPiece().getColor() != this.getColor() && !(board[row - 1][col].getOccupyingPiece() instanceof Unmovable)) {
		...
		if (row < board.length - 1) {
			if (!board[row +1][col].isOccupied()
					|| board[row+1][col].getOccupyingPiece().getColor() != this.getColor() && !(board[row + 1][col].getOccupyingPiece() instanceof Unmovable)) {
				legalMoves.add(board[row+1][col]);
			}
		}
		//right
